...
from typing import Sequence, List, Union, Tuple, Any, Callable, Dict, Optional
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        self,
        dataset_path: Union[Path, str],  # MODIFIED: Replaced Path | str
        modality_configs: Dict[str, ModalityConfig],
        embodiment_tag: Union[str, EmbodimentTag],  # MODIFIED: Replaced str | EmbodimentTag
        video_backend: str = "decord",
        video_backend_kwargs: Optional[dict] = None,  # MODIFIED: Replaced dict | None
        transforms: Optional[ComposedModalityTransform] = None,  # MODIFIED: Replaced ... | None
    ):
        """
        ...
        if isinstance(embodiment_tag, EmbodimentTag):
            ...
            # can be directly a string (name of the embodiment tag)
            self.tag = embodiment_tag

        self._metadata = self._get_metadata(EmbodimentTag(self.tag))
        # _trajectory_ids and _trajectory_lengths are 1D numpy arrays
        # _trajectory_ids is an array of integers, identifying each trajectory
        # _trajectory_lengths is an array of integers, indicating the length of each trajectory (number of steps)
        self._trajectory_ids, self._trajectory_lengths = self._get_trajectories()
        # List[tuple[int,int]]: list of (trajectory_id, base_index) tuples for all steps in the dataset
        # e.g. trajectory_ids: [0, 1, 2], trajectory_lengths: [3, 2, 4]
        # return: [
        #     (0, 0), (0, 1), (0, 2),
        #     (1, 0), (1, 1),
        #     (2, 0), (2, 1), (2, 2), (2, 3)
        ...
        # Dict[str, List[str]]: keys are modality names, values are lists of modality keys
        # this is from the modality configs
        self._modality_keys = self._get_modality_keys()
        # keys are modality.key, values are delta indices
    ...
    def get_step_data(self, trajectory_id: int, base_index: int) -> dict:
        ...
        # this is a pd.DataFrame from the parquet file for the trajectory
    ...
    def get_trajectory_data(self, trajectory_id: int) -> pd.DataFrame:
        ...
        if self.curr_traj_id == trajectory_id and self.curr_traj_data is not None:
            ...
            # dataset was splited into chunks of self.chunk_size episodes
            # we have to find the chunk index for the episode index (trajectory_id // chunk_size)
            chunk_index = self.get_episode_chunk(trajectory_id)
            parquet_path = self.dataset_path / self.data_path_pattern.format(
                episode_chunk=chunk_index, episode_index=trajectory_id
            )
            assert parquet_path.exists(), f"Parquet file not found at {parquet_path}"
            self.curr_traj_id = trajectory_id # TODO: this may fix bug? Or cause some bug?
    ...
    def get_video(
        ...
        # print(f"getting video from {video_path},  timestamp: {video_timestamp} kwargs: {self.video_backend_kwargs}")
    ...
    def get_state_or_action(
        ...
        # Pad the data
        # print(f"state_or_action_cfg.absolute: {state_or_action_cfg.absolute}")
    ...
    def get_language(
        ...
        step_indices = self.delta_indices[key] + base_index
        # Get the trajectory index
        trajectory_index = self.get_trajectory_index(trajectory_id)  # trajectory_id is identity of the trajectory, trajectory_index is the index in self.trajectory_ids
        # Get the maximum length of the trajectory
        max_length = self.trajectory_lengths[trajectory_index]
...
class CachedLeRobotSingleDataset(LeRobotSingleDataset):
    def __init__(self, img_resize: Optional[Tuple[int, int]] = None, *args, **kwargs):  # MODIFIED: Replaced tuple[int, int] | None
        """
...
class MixtureSpecElement(BaseModel):
    dataset_path: Union[List[Path], Path] = Field(..., description="The path to the dataset.")  # MODIFIED: Replaced List[Path] | Path
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        self,
        data_mixture: Sequence[Tuple[LeRobotSingleDataset, float]],  # MODIFIED: Replaced tuple with Tuple
        mode: str,
    ...
    def compute_overall_statistics(
        per_task_stats: List[Dict[str, Dict[str, Union[List[float], np.ndarray]]]],  # MODIFIED: Replaced |
        dataset_sampling_weights: Union[List[float], np.ndarray],  # MODIFIED: Replaced |
