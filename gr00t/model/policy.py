...
class Gr00tPolicy(BasePolicy):
    ...
    @property
    # MODIFIED for Python 3.8: Replaced `np.ndarray | None` with `Optional[np.ndarray]`
    ...
    def _load_metadata(self, exp_cfg_dir: Path):
        ...
        # --- 开始插入修改的代码 ---
        # 
        # 在 Pydantic 验证之前，手动将 statistics 字典中的 list 转换为 numpy array
        if 'statistics' in metadata_dict:
            stats = metadata_dict['statistics']

            if 'state' in stats:
                for state_key, state_stats in stats['state'].items():
                    for stat_name, value in state_stats.items():
                        if isinstance(value, list):
                            stats['state'][state_key][stat_name] = np.array(value)

            if 'action' in stats:
                for action_key, action_stats in stats['action'].items():
                    for stat_name, value in action_stats.items():
                        if isinstance(value, list):
                            stats['action'][action_key][stat_name] = np.array(value)
        # --- 结束插入修改的代码 ---
