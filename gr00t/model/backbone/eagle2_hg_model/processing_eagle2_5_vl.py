...
MODIFIED FOR transformers==4.38.2 and python==3.8
"""
...
# <--- MODIFIED: 导入 Dict, Tuple, Union 等
...
# <--- MODIFIED: 移除 'VideoInput'
from transformers.image_utils import ImageInput
# <--- MODIFIED: 移除 'ProcessingKwargs' 和 'Unpack'
...
# <--- MODIFIED: 移除 'Eagle2_5_VLProcessorKwargs'
...
class Eagle2_5_VLProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # <--- MODIFIED: 修复 TypeError
        # 'ProcessorMixin' in v4.38.2 does not accept 'chat_template' in its __init__
        # 移除 'chat_template=chat_template'
    ...
    def replace_media_placeholder(
        ...
        def replace_in_text(text):
            ...
            def repl(match):
                ...
                if media_type == "image":
                    # <--- MODIFIED: 强制返回 'pt' 张量
                    local_images_kwargs = output_kwargs["images_kwargs"].copy()
                    local_images_kwargs["return_tensors"] = "pt"
                    # --- 结束修正 ---
                    image_inputs = self.image_processor(
                        images=[image_list[idx_in_list]],
                        videos=None,
                        **local_images_kwargs, # <--- MODIFIED: 使用本地 kwargs
                    )
                    ...
                    local_videos_kwargs = output_kwargs["videos_kwargs"].copy()
                    local_videos_kwargs["return_tensors"] = "pt"
                    # --- 结束修正 ---
                    video_inputs = self.image_processor(
                        images=None,
                        videos=[video_list[idx_in_list]],
                        **local_videos_kwargs, # <--- MODIFIED: 使用本地 kwargs
                    )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # <--- MODIFIED: Manwal na pag-handle ng kwargs
        text_kwargs = kwargs.pop("text_kwargs", {"padding": False})
        images_kwargs = kwargs.pop("images_kwargs", {})
        videos_kwargs = kwargs.pop("videos_kwargs", {"max_dynamic_tiles": 1})

        # Idagdag ang anumang natitirang kwargs sa text_kwargs para sa tokenizer
        text_kwargs.update(kwargs)

        # Gumawa ng dict para sa media processing
        media_kwargs = {
        ...
        # <--- MODIFIED: Gamitin ang 'videos_kwargs' variable
        timestamps_batch = videos_kwargs.pop("timestamps", None)
        fps_batch = videos_kwargs.pop("fps", None)
        ...
        for sample in text_list:
            ...
            (
                ...
                timestamps_list,
                fps_list,
                # <--- MODIFIED: Ipasa ang ginawang 'media_kwargs' dict
                **media_kwargs,
            )
        ...
        # <--- MODIFIED: Gamitin ang 'text_kwargs' variable
        text_inputs = self.tokenizer(new_sample_list, **text_kwargs)
    ...
    # <--- MODIFIED: Python 3.8 语法
        ...
        Optional[List[Image.Image]], Optional[List[Union[torch.Tensor, List[Image.Image]]]], Optional[Dict]
    ]:
    ...
    def from_args_and_dict(cls, args, processor_dict: Dict[str, Any], **kwargs):
        ...
        # <--- MODIFIED: 移除 'validate_init_kwargs'
