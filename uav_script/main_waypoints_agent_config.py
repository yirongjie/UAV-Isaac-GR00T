...
# --- [新增] 配置文件路径 ---
CONFIG_PATH = "config_waypoints_agent.json"
# ----------------------------------------------------
...
# --- [新增函数] 加载任务配置 ---
def load_mission_config(config_path):
    """从 JSON 文件加载航点和 Agent 指令配置"""
    ...
    if not waypoints:
        print("警告: 配置文件中未找到 'waypoints'。航点任务将被跳过。")
    if not agent_commands:
        print("警告: 配置文件中未找到 'agent_commands'。Agent 任务将无法执行。")
        
    return waypoints, agent_commands
# --- [新增函数结束] ---


# --- [关键修改] 采用用户提供的 run_waypoint_mission 逻辑，并从配置中读取航点 ---
def run_waypoint_mission(drone, relative_waypoints: list):
    """
    执行航点任务，起飞并飞行，结束后悬停。
    relative_waypoints: 从配置中读取的 [[东向(m), 北向(m)], ...]
    """
    
    if not relative_waypoints:
        print("--- [阶段 1: 航点任务] ---")
        print("没有配置航点，跳过航点飞行阶段。")
        return True
    ...
    try:
        ...
        for i, (dx_m, dy_m) in enumerate(relative_waypoints):
            # 使用当前点计算下一个绝对 GPS 坐标
            next_lat, next_lon = calculate_new_gps(current_lat, current_lon, dx_m, dy_m)
            
            mission_points_lla.append([next_lat, next_lon, base_alt])
            
            print(f"  -> 航点 {i+1} (相对位移: 东{dx_m}m, 北{dy_m}m): Lat={next_lat:.6f}, Lon={next_lon:.6f}")
            
            # 更新当前点为下一个点的坐标，用于下一次计算
...
def agent_main(agent_commands: list):
    """
    配置并运行 Agent 的主函数。
    修改为循环读取 JSON 配置文件中的任务指令。
    """
    try:
        ...
        print("\n--- Agent 初始化完成，开始执行配置文件中的指令 ---")

        # 2. 循环执行配置文件的 Agent 指令
        for i, prompt in enumerate(agent_commands):
            if stop_event.is_set():
                break

            print(f"\n--- 正在执行 Agent 指令 {i+1}/{len(agent_commands)}: **{prompt}** ---")
            agent.run(prompt) 
...
if __name__ == "__main__":
    ...
    # 0. 加载配置
    try:
        relative_waypoints, agent_commands = load_mission_config(CONFIG_PATH)
        print(f"成功加载配置: {len(relative_waypoints)} 个航点, {len(agent_commands)} 条 Agent 指令。")
    except Exception as e:
        print(f"!!! 致命错误: 加载任务配置文件失败: {e}")
        exit(1)
    ...
    try:
        ...
        waypoint_success = run_waypoint_mission(drone, relative_waypoints)
        ...
        if not waypoint_success:
            ...
            if DRONE_TYPE == "tello":
                ...
                # 将 Agent 任务作为参数传入
                agent_main_thread = threading.Thread(target=agent_main, args=(agent_commands,), daemon=True)
